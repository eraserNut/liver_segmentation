...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = Conv1x1GNReLU(in_channels, out_channels, upsample=False)
        self.conv2 = Conv3x3GNReLU(out_channels, out_channels, upsample=False)
        self.conv3 = nn.Sequential(nn.Conv2d(in_channels, out_channels, (1, 1), stride=1, padding=0, bias=False),
...
class DSBlock(nn.Module):
    def __init__(self, in_channels, final_channels):
        super().__init__()
        # ==> FN Sub-module
        self.fn_extra = nn.Sequential(Conv3x3GNReLU(in_channels, in_channels),
                                      Conv3x3GNReLU(in_channels, in_channels))
        self.fn_attention = nn.Sequential(
            nn.Conv2d(in_channels*2, 1, (3, 3), stride=1, padding=1, bias=False),
            nn.Sigmoid(),
        )
        # self.fn_final = nn.Conv2d(in_channels, final_channels, (3, 3), stride=1, padding=1, bias=False)

        # ==> FP Sub-module
        self.fp_extra = nn.Sequential(Conv3x3GNReLU(in_channels, in_channels),
                                      Conv3x3GNReLU(in_channels, in_channels))
        self.fp_convBlock = nn.Sequential(
            ResidualBlock(in_channels * 2, in_channels * 2),
            Conv1x1GNReLU(in_channels * 2, in_channels),
            Conv3x3GNReLU(in_channels, in_channels),
            Conv1x1GNReLU(in_channels, in_channels),
        )
    ...
    def forward(self, x):
        # ==> FN Sub-module
        fnd = self.fn_extra(x)
        # fn_pred = self.fn_final(fnd) #return
        fnd_fuse = torch.cat([fnd, x], dim=1)
        f_mask = self.fn_attention(fnd_fuse)
        fnd_ = x * f_mask.expand(x.size())
        x_fn = fnd_ + x

        # ==> FN Sub-module
        fpd = self.fp_extra(x)
        # fp_pred = self.fp_final(fpd) #return
        fpd_fuse = torch.cat([fpd, x_fn], dim=1)
        fpd_ = self.fp_convBlock(fpd_fuse)
        x_ds = x_fn - fpd_
        return x_ds, fnd, fpd, f_mask
...
class FPN_DS(Model):
    ...
    def forward(self, x):
        ...
        # ==> DS module
        d5, fnd5, fpd5, mask5 = self.d5(c5)
        d4, fnd4, fpd4, mask4 = self.d4(c4)
        d3, fnd3, fpd3, mask3 = self.d3(c3)
        d2, fnd2, fpd2, mask2 = self.d2(c2)
        ...
        if self.training:
            ...
            # return F.sigmoid(x), [F.sigmoid(x_fnd)]
