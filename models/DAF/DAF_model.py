...
from ..backbone import ResNeXt101
...
class DAF(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return [torch.sigmoid(predict1), torch.sigmoid(predict2), torch.sigmoid(predict3),
                    torch.sigmoid(predict4), torch.sigmoid(predict1_2), torch.sigmoid(predict2_2),
                    torch.sigmoid(predict3_2), torch.sigmoid(predict4_2)]
        else:
