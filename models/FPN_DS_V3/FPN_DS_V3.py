...
class FPN_DS_V3(Model):
    ...
    def __init__(
        ...
        self.fnd_m5 = modifyBlock(3, 'strengthen', 1)
        self.fnd_m4 = modifyBlock(2, 'strengthen', 1)
        self.fnd_m3 = modifyBlock(1, 'strengthen', 1)
        self.fnd_m2 = modifyBlock(0, 'strengthen', 1)

        # ==> DS module, weaken block
        self.fpd_m5 = modifyBlock(3, 'weaken', 1)
        self.fpd_m4 = modifyBlock(2, 'weaken', 1)
        self.fpd_m3 = modifyBlock(1, 'weaken', 1)
        self.fpd_m2 = modifyBlock(0, 'weaken', 1)
    ...
    def forward(self, x):
        ...
        fnd_mask = self.relu(x_fnd)/(torch.max(x_fnd)+1e-8)
        fpd_mask = self.relu(x_fpd)/(torch.max(x_fpd)+1e-8)
        # fnd_mask = self.sig_instanceNormal(x_fnd)
        # fpd_mask = self.sig_instanceNormal(x_fpd)
